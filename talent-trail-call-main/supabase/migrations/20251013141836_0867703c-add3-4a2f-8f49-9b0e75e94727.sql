-- Create a function to find and match users
...
-- Create trigger to run the matching function
DROP TRIGGER IF EXISTS trigger_match_users ON public.waiting_room;
CREATE TRIGGER trigger_match_users
  AFTER INSERT ON public.waiting_room
  FOR EACH ROW
  EXECUTE FUNCTION public.match_waiting_users();